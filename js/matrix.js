...
function Go() {
    ...
    let
    day = Number(EnterDay),
    month = Number(EnterMonth),
    year = Number(EnterYear);
        ...
        if (year < 1920 || year <= 0 || year > 2100 ) {
            throw new Error(alert('Число не должно быть меньше 1920 или больше 2100'));
        };

        //Разбираю day на запчасти. Пример: 13 (1 , 3) 
        let a = Math.floor(day / 10);
        let b = day - (a * 10);

        //Разбираю month на запчасти. Пример: 11 (1 , 1)
        let c = Math.floor(month / 10);
        let d = month - (c * 10);

        //Разбираю year на запчасти. Пример: 1987 (1 , 9 , 8 , 7)
        let e = Math.floor(year / 1000);//1
        let e1 = year - (e * 1000);       
        ...
        //Создаю массив
        let arrayNumberStr1 = [day,month,year];
        ...
        let matrixWrepperID = matrixDay + '.' + ' ' + matrixMonth + '.' + ' ' + year;
